...
do_lipo () {
    ...
    do
        LIPO_FLAGS="$LIPO_FLAGS $UNI_BUILD_ROOT/build/speexdsp-$ARCH/output/lib/$LIB_FILE"
    done
...
do_lipo_all () {
    ...
    cp -R $UNI_BUILD_ROOT/build/speexdsp-armv7/output/include $UNI_BUILD_ROOT/build/universal/
}

#----------
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh tools/do-compile-speexdsp.sh $FF_TARGET
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh tools/do-compile-speexdsp.sh $ARCH
    done
    ...
    echo "  compile-speexdsp.sh armv7s (obselete)"
    echo "  compile-speexdsp.sh lipo"
    echo "  compile-speexdsp.sh all"
    echo "  compile-speexdsp.sh clean"
    echo "  compile-speexdsp.sh check"
    exit 1
fi
